...
async def get_user_listings(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
...
async def remove_listing(listing_id, user_id):
    """Faqat egasi o'chira oladi. Muvaffaqiyatli bo'lsa True qaytaradi."""
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "UPDATE listings SET status = 'removed' WHERE listing_id = ? AND user_id = ?",
            (listing_id, user_id)
        )
        await db.commit()
...
async def get_active_jobs(job_type, region=None):
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        if region:
            cur = await db.execute("""
                SELECT * FROM jobs WHERE job_type = ? AND status = 'active' AND location = ?
                ORDER BY created_at DESC
            """, (job_type, region))
        else:
            cur = await db.execute("""
...
async def remove_job(job_id, user_id):
    """Faqat egasi o'chira oladi. Muvaffaqiyatli bo'lsa True qaytaradi."""
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "UPDATE jobs SET status = 'removed' WHERE job_id = ? AND user_id = ?",
            (job_id, user_id)
        )
        await db.commit()
        return cur.rowcount > 0
