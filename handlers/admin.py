...
async def admin_give_money_amount(message: Message, state: FSMContext, bot: Bot):
    ...
    if not text.lstrip("-").isdigit():
        await message.answer("⚠️ Summani faqat raqamlarda kiriting:")
        return
    ...
    try:
        await bot.send_message(
            target_id,
            f"💰 Balansingizga admin tomonidan <b>{fmt(amount)} so'm</b> qo'shildi.",
            parse_mode="HTML"
        )
    except Exception:
        pass
...
async def admin_broadcast_id(message: Message, state: FSMContext):
    ...
    if text.lower() != "all" and not text.isdigit():
        await message.answer("⚠️ ID faqat raqamlardan iborat bo'lishi kerak yoki \"all\" deb yozing:")
        return
    await state.update_data(target=text)
    await state.set_state(AdminBroadcast.entering_message)
    await message.answer("✏️ Yubormoqchi bo'lgan xabar matnini kiriting:")
    ...
    if target == "all":
        ...
        for u in users:
            try:
                ...
                pass
        await message.answer(
            f"✅ Xabar <b>{sent}</b> ta foydalanuvchiga yuborildi.",
            reply_markup=admin_reply_kb(),
            parse_mode="HTML"
        )
    else:
        try:
            ...
            await message.answer(
                "⚠️ Xabar yuborib bo'lmadi. Foydalanuvchi botni bloklagan bo'lishi mumkin.",
                reply_markup=admin_reply_kb()
            )
...
async def admin_stats(message: Message):
    ...
    await message.answer(
        ...
        "━━━━━━━━━━━━━━━━━━━\n"
        f"👥 A'zolar: <b>{users_count}</b> ta\n"
        f"💰 Jami kiritilgan pul: <b>{fmt(total_deposits)} so'm</b>\n"
        f"📦 Faol e'lonlar: <b>{listings_count}</b> ta\n"
        "━━━━━━━━━━━━━━━━━━━",
        parse_mode="HTML"
    )
