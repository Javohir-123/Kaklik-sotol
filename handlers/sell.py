...
from data_lists import get_category_title, get_item_title, CATEGORIES_WITH_GENDER
...
async def sell_enter_phone(message: Message, state: FSMContext):
    ...
    gender_line = f"⚧ Jinsi: {data['gender']}\n" if cat_key in CATEGORIES_WITH_GENDER else ""
...
async def sell_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if needs_payment and user["balance"] < LISTING_PRICE:
        ...
        price_fmt = f"{LISTING_PRICE:,}".replace(",", " ")
        await callback.message.edit_text(
            "💳 <b>Balans yetarli emas</b>\n"
            "━━━━━━━━━━━━━━━━━━━\n"
            f"E'lon joylash uchun: <b>{price_fmt} so'm</b>\n"
            f"Sizning balansingiz: <b>{balance_fmt} so'm</b>\n"
            "━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    if needs_payment:
        price_fmt = f"{LISTING_PRICE:,}".replace(",", " ")
        extra_line = f"💳 Balansingizdan {price_fmt} so'm yechildi."
    else:
